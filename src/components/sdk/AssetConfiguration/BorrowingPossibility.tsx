...
export const BorrowingPossibility = () => {
  const { sdk } = useSDK();
  ...
  const onChangePossibility = async () => {
          ...
          <AccordionPanel pb={4}>
            <Flex direction="column" gap={4}>
              <Flex width="100%" direction="row" gap={4}>
                ...
                {poolId && (
                  <>
                    ...
                    {selectedAsset && (
                      <FormControl flex={3}>
                        <FormLabel mt={10}>{`This asset can ${
                          selectedAsset.isBorrowPaused ? 'not' : ''
                        } be borrowed`}</FormLabel>
                      </FormControl>
                    )}
                  </>
                )}
              ...
              <Button
                ...
                width="100px"
                colorScheme="teal"
                onClick={onChangePossibility}
                isLoading={isStatus}
                isDisabled={isStatus || !selectedAsset}
              >
                {selectedAsset?.isBorrowPaused ? 'Resume' : 'Pause'}
              </Button>
            </Flex>
          </AccordionPanel>
        </AccordionItem>
