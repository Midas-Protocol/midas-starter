...
const AmountSelect = ({
  ...
  const { address, fuse, chainId, setPendingTxHash } = useRari();
  ...
  const { cSolidBtn, cCard, cSwitch } = useColors();
  ...
  const onConfirm = async () => {
    try {
      ...
      const isRepayingMax = amount.eq(asset.borrowBalance) && !isNativeToken && mode === Mode.REPAY;
      ...
      if (mode === Mode.SUPPLY || mode === Mode.REPAY) {
        ...
        if (mode === Mode.SUPPLY) {
          ...
          if (isNativeToken) {
            ...
            if (
              ...
              tx = await call({ from: address, value: amount });
            }
          } else {
            //  Custom amount of ERC20
            tx = await testForCTokenErrorAndSend(
        ...
        // );
        tx = await cToken.redeemUnderlying(amount);
